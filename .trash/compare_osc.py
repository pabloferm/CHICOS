...
E_vals = np.linspace(0.1, 5, 100)  # Energy in GeV (avoid zero to prevent division issues)
...
for i in range(3):
    for j in range(3):
        ax = axes[i, j]
        ax.plot(E_vals, P_diff_exact_approx[:, i, j], label="Exact - Approx", linestyle='-')
        ax.plot(E_vals, P_diff_exact_nusquids[:, i, j], label="Exact - nuSQuIDS", linestyle='--')
